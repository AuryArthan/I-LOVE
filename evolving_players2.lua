...
function Player:mutate_player()
	for i = 1,8 do
		self.weights[i] = self.weights[i]*(1+(math.random()-0.5)/20)		-- small mutation
		if math.random() < 0.5 then
			self.weights[i] = self.weights[i]*(1+(math.random()-0.5)/10)	-- bigger mutation
		end
		if math.random() < 0.2 then
			self.weights[i] = self.weights[i]*(1+(math.random()-0.5)/5)		-- big mutation
		end
		if math.random() < 0.1 then
			self.weights[i] = self.weights[i]*(1+(math.random()-0.5)/2)		-- huge mutation
		end
	end
end
...
for n = 1,N do
	...
	for r = 1,16 do players[n]:mutate_player() end
end
...
-- init output file
local file = io.open("trained_players2/output.txt", "w")
file:close()
-- generation loop
for gen = 1,Ngen do
	...
	file = io.open("trained_players2/output.txt", "a")
	for p = 1,60 do
