...
for n = 1,N do
	...
	for r = 1,50 do players[n]:mutate_player() end
end
...
-- init output file
local file = io.open("trained_players3/output.txt", "w")
file:close()
-- generation loop
for gen = 1,Ngen do
	...
	-- the top 50 reproduce and have 3 mutated offspring each
	for p = 1,50 do
		players[50+p] = players[p]:copy()
		players[100+p] = players[p]:copy()
		players[150+p] = players[p]:copy()
		...
		for r = 1,3 do players[150+p]:mutate_player() end		-- 3rd offspring tripple mutation
	end
	-- write the top 50 to a file
	file = io.open("trained_players3/output.txt", "a")
	for p = 1,50 do
